  ...
  test('fragment properties that are set to null are indicated in the owner record\'s `changedAttributes`', function(assert) {
    run(() => {
      ...
      return store.find('person', 1).then(person => {
        ...
        expectedChanges.push(person.get('name'));
        person.set('name', null);         person.set('name', null);
        expectedChanges.push(person.get('name'));

        assert.equal(person.changedAttributes().name, true, 'null fragments are indicated in the diff object');          assert.deepEqual(person.changedAttributes().name, expectedChanges, 'null fragments are indicated in the diff object');      });
    });
  });
